...
try {
  ...
  const componentLinks = db.prepare(`
    SELECT cmp_id FROM pricing_pages_cmps 
  ...
  for (let i = 0; i < pricingData.plans.length; i++) {
    ...
    // Link component to page (Strapi 5 schema: cmp_id)
    db.prepare(`
      INSERT INTO pricing_pages_cmps (entity_id, cmp_id, component_type, field, [order])
      VALUES (?, ?, ?, ?, ?)
    `).run(pageId, cmpId, 'pricing.tier', 'pricingTiers', i + 1);
  }
