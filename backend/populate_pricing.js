...
try {
  ...
  if (existingPage) {
    ...
    db.prepare(`
      UPDATE pricing_pages 
      SET hero_headline = ?, hero_subheadline = ?, hero_description_left = ?, hero_description_right = ?
      WHERE id = ?
    `).run(
      pricingData.hero_headline,
      pricingData.hero_subheadline,
      pricingData.description_left,
      pricingData.description_right,
      pageId
    );
    console.log('Updated Pricing Page hero content.');
  } else {
    const result = db.prepare(`
      INSERT INTO pricing_pages (hero_headline, hero_subheadline, hero_description_left, hero_description_right, published_at, created_at, updated_at)
  ...
  // 2. Clear existing Pricing Tiers for this page to avoid duplicates
  // Note: Repeatable components in Strapi 5 are stored in components_pricing_tiers 
  // and linked via pricing_pages_components
  
  // First, find the component links
  const componentLinks = db.prepare(`
    SELECT component_id FROM pricing_pages_components 
    WHERE entity_id = ? AND component_type = 'pricing.tier'
  `).all(pageId);
  
  for (const link of componentLinks) {
    db.prepare('DELETE FROM components_pricing_tiers WHERE id = ?').run(link.component_id);
  }
  db.prepare('DELETE FROM pricing_pages_components WHERE entity_id = ? AND component_type = "pricing.tier"').run(pageId);
  ...
  for (let i = 0; i < pricingData.plans.length; i++) {
    ...
    const compResult = db.prepare(`
      INSERT INTO components_pricing_tiers (package_name, package_tagline, package_price, package_features, button_label, button_url)
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(
      plan.name,
      plan.tagline,
    ...
    db.prepare(`
      INSERT INTO pricing_pages_components (entity_id, component_id, component_type, field, order)
      VALUES (?, ?, ?, ?, ?)
    `).run(pageId, componentId, 'pricing.tier', 'pricingTiers', i + 1);
  }
