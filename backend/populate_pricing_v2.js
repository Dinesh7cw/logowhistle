...
try {
  ...
  if (existingPage) {
    ...
    // Map property names (camelCase in JSON -> snake_case in SQLite)
    db.prepare(`
      UPDATE pricing_pages 
      SET hero_headline = ?, hero_subheadline = ?, hero_description_left = ?, hero_description_right = ?, updated_at = datetime('now')
      WHERE id = ?
    `).run(
    ...
    // Note: Strapi 5 might require document_id
    const documentId = Math.random().toString(36).substring(2, 12);
    const result = db.prepare(`
      INSERT INTO pricing_pages (hero_headline, hero_subheadline, hero_description_left, hero_description_right, document_id, published_at, created_at, updated_at)
  ...
  for (let i = 0; i < pricingData.plans.length; i++) {
    ...
    // Link component to page (Strapi 5 schema)
    db.prepare(`
